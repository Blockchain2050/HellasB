...
class LinkPreview extends \OssnObject
{
	...
	{
		return $this->isChinese($string) || $this->isJapanese($string) || $this->isKorean($string) || $this->isGreek($string);
	}
	...
	public function isGreek($string)
	// add preg_replace to remove special characters that create problems when reads meta data
	{	error_log(preg_match("/[^a-z0-9\x{3B1}-\x{3C9}-\x{a0}]+/u", $string));
		// $string = preg_replace('/\b(?:&nbsp;|#)\b/', '', $string);
		return preg_match("/[^a-z0-9\x{3B1}-\x{3C9}-\x{a0}]+/u", $string);
	}
	/**
