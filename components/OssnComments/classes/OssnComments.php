...
class OssnComments extends OssnAnnotation {
		...
		public function PostComment($subject_id, $guid, $comment, $type = 'post') {
				...
				$this->value        = $comment;
				// remove points for every comment
				$this->statement("SELECT points FROM tk_points WHERE (guid='{$this->owner_guid}');");
				$this->execute();
				$this->check = $this->fetch();
				$points = $this->check->points - 2;
				$this->statement("UPDATE tk_points SET points='{$points}' WHERE(guid='{$this->owner_guid}');");
				$this->execute();
				// -----------------------
